...
date +"START  %c"
C
C --- Create 1hrly interannual forcing from 1hrly climo forcing.
C ---
...
if ( $flxflg != 5 ) then
  foreach f ( airtmp vapmix  )
else
  foreach f ( airtmp spchum  )
endif
#foreach f ( airtmp vapmix radflx shwflx precip )
#foreach f ( airtmp vapmix radflx shwflx )
#foreach f ( airtmp vapmix )
#foreach f ( airtmp spchum )
  touch ${f}.a
  if (-z ${f}.a) then
...
# JDA is CICE dimension, JDM-1 for tripole grids
  ...
  cd ../cice
  /bin/rm -f vapmix_${Y01}${A}.r airtmp_${Y01}${A}.a airtmp_${Y01}${A}.r 
  if ( $flxflg != 5 ) then
   ${BINRUN} ~/HYCOM-tools/bin/hycom2raw8 ../flux/vapmix_${Y01}${A}.a ${IDM} ${JDM} 1 1 ${IDM} ${JDA} vapmix_${Y01}${A}.r >! vapmix_${Y01}${A}.B
  else
# spchum is still called vapmix for CICE
   ${BINRUN} ~/HYCOM-tools/bin/hycom2raw8 ../flux/spchum_${Y01}${A}.a ${IDM} ${JDM} 1 1 ${IDM} ${JDA} vapmix_${Y01}${A}.r >! vapmix_${Y01}${A}.B
  endif
# airtmp is in degK
  ${BINRUN} ${TOOS}/bin/hycom_expr ../flux/airtmp_${Y01}${A}.a ONE ${IDM} ${JDM} 1.0 273.15 airtmp_${Y01}${A}.a >! airtmp_${Y01}${A}.b
...
C
C  --- END OF JUST IN TIME FLUX EXTRACTION SCRIPT.
C
date +"END    %c"
#
