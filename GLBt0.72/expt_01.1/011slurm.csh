...
C --- Submit with msub, or msub_csh, or msub_slurm_wrapper command.
...
C
if ( -e RUNNING && ! -e RUNNING_${SLURM_JOB_ID}) then
C
...
C
  /bin/rm -f RUNNING
  /bin/rm -f RUNNING_${SLURM_JOB_ID}
  exit
endif
C
...
#       /bin/nice -7 csh ${E}pbs.csh >& ${newname}.log &
        csh ${E}slurm.csh >& ${newname}.log &
    else
#       kinit -R
#       qsub -N R${newname} -j oe -o R${newname}.log ${E}pbs.csh
#             -j oe -o R${newname}.log < ${E}pbs.csh
        sed -e "s/^#SBATCH  *-J .*/#SBATCH -J R${newname}/" -e "s/^#SBATCH  *-o .*/#SBATCH -o R${newname}.log/" ${E}slurm.csh >! ${E}slurm_$$.csh
        sbatch  ${E}slurm_$$.csh
        /bin/rm ${E}slurm_$$.csh
    endif
else
