...
date +"START  %c"
C
C --- Create model interpolated mslprs for HYCOM.
C --- From NWP NRL .nc files.
C
C --- prebuild this script similar to a model script
# --- awk -f 284.awk y01=098 ab=a 284M.csh > 284m098a.csh
C
...
setenv CDF071 ${N}-sec_${YC1}_01hr_sfcprs.nc
setenv CDF072 ${N}-sec_${YCX}_01hr_sfcprs.nc
C
touch  $CDF071
if (-z $CDF071) then
  ${pget} ${W}/$CDF071 . &
...
cat <<E-o-D  > fort.05i
 ...
 &AFTIME
  ...
  TSTART = ${TS},
  TMAX   = ${TM},
  PARMIN = -9.e10,  !disable parmin
  PARMAX =  9.e10,  !disable parmax
  PARSCL =  100.00, !hPa to Pa
  PAROFF =    0.00, !already -1000 hPa
 /
 &AFFLAG
...
E-o-D
C
C --- run the mslprs interpolation.
C
date +"mslprs %c"
...
C --- Output.
C
/bin/mv fort.10  mslprs_${Y01}${A}.b
/bin/mv fort.10a mslprs_${Y01}${A}.a
C
if (-e ./SAVE) then
  ln mslprs_${Y01}${A}.a ./SAVE/mslprs_${Y01}${A}.a
  ln mslprs_${Y01}${A}.b ./SAVE/mslprs_${Y01}${A}.b
endif
C
C  --- END OF JUST IN TIME MSLPRS GENERATION SCRIPT.
C
date +"END    %c"
