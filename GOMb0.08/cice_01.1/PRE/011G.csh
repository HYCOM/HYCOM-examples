...
#   global tripole region
  ...
  if (CICE_TYPE == 0) then
    ...
    touch  regional.grid.a regional.grid.b
    if (-z regional.grid.b) then
      ln -f ../grad/regional.grid.b regional.grid.b
      ln -f ../grad/regional.grid.a regional.grid.a
    endif
    setenv FOR020    ../grad/glbrad_${Y01}${A}.b
    setenv FOR020A   ../grad/glbrad_${Y01}${A}.a
    setenv CDF_TITLE $N
    setenv CDF_INST  "COAPS/FSU"
    setenv CDF_FILE  glbrad_${Y01}${A}.nc
    /bin/rm -rf $CDF_FILE
    ${BINRUN} ~/HYCOM-tools/force/src/force2nc
    if (-e ./SAVE) then
      foreach f ( glbrad_${Y01}${A}.nc )
        ln ${f} ./SAVE/${f}
      end
    endif
  endif
