...
case 'unicos':
case 'unicosmk':
    if (-e        ~${user}/bin/pget-IGNORE) then
...
#   global tripole region
  ...
  touch  regional.grid.a regional.grid.b
  if (-z regional.grid.b) then
    ln -f ../flux/regional.grid.b regional.grid.b
    ln -f ../flux/regional.grid.a regional.grid.a
  endif
# airtmp is in degK
  /bin/rm -f airtmp_${Y01}${A}.a
  ${BINRUN} ~/HYCOM-tools/bin/hycom_expr ../flux/airtmp_${Y01}${A}.a ONE ${IDM} ${JDM} 1.0 273.15 airtmp_${Y01}${A}.a | grep min | sed -e "s/min, max =//g" >! airtmp_${Y01}${A}.B2
  tail -n +6 ../flux/airtmp_${Y01}${A}.b | cut -c 1-50  >! airtmp_${Y01}${A}.B1
  head -n  5 ../flux/airtmp_${Y01}${A}.b                >! airtmp_${Y01}${A}.b
  paste -d" " airtmp_${Y01}${A}.B1 airtmp_${Y01}${A}.B2 >> airtmp_${Y01}${A}.b
  /bin/rm     airtmp_${Y01}${A}.B1 airtmp_${Y01}${A}.B2
#
 if (CICE_TYPE == 0) then
    /bin/rm -f vapmix_${Y01}${A}.r airtmp_${Y01}${A}.r
    ...
    /bin/rm -rf fort.20 fort.20A
    ln -sf airtmp_${Y01}${A}.b fort.20
    ln -sf airtmp_${Y01}${A}.a fort.20A
    setenv FOR020A   fort.20A
    setenv CDF_TITLE $N
    setenv CDF_INST  "COAPS/FSU"
    setenv CDF_PNAME " air temperature  "
    setenv CDF_SNAME "air_temperature"
    setenv CDF_UNAME "degK"
    setenv CDF_FILE  airtmp_${Y01}${A}.nc
    /bin/rm -rf $CDF_FILE
    ${BINRUN} ~/HYCOM-tools/force/src/force2nc
    /bin/rm -rf fort.20 fort.20A
    ln -sf ../flux/vapmix_${Y01}${A}.b fort.20
    ln -sf ../flux/vapmix_${Y01}${A}.a fort.20A
    setenv FOR020A   fort.20A
    setenv CDF_TITLE $N
    setenv CDF_INST  "COAPS/FSU"
    setenv CDF_FILE  vapmix_${Y01}${A}.nc
    /bin/rm -rf $CDF_FILE
    ${BINRUN} ~/HYCOM-tools/force/src/force2nc
    if (-e ./SAVE) then
      foreach f ( vapmix_${Y01}${A}.nc airtmp_${Y01}${A}.nc )
        ln ${f} ./SAVE/${f}
      end
    endif
  endif
