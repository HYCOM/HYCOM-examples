...
#   global tripole region
  ...
  cd ../cice
  foreach t ( wndewd wndnwd )
    if (CICE_TYPE == 0) then
      /bin/rm ${t}_${Y01}${A}.r
      ${BINRUN} ~/HYCOM-tools/bin/hycom2raw8 ../wvel/${t}_${Y01}${A}.a ${IDM} ${JDM} 1 1 ${IDM} ${JDA} ${t}_${Y01}${A}.r >! ${t}_${Y01}${A}.B
      if (-e ./SAVE) then
        foreach f ( ${t}_${Y01}${A}.[rB] )
          ln ${f} ./SAVE/${f}
        end
      endif
    else
      touch  regional.grid.a regional.grid.b
      if (-z regional.grid.b) then
        ln -f ../wvel/regional.grid.b regional.grid.b
        ln -f ../wvel/regional.grid.a regional.grid.a
      endif
      setenv FOR020    ../wvel/${t}_${Y01}${A}.b
      setenv FOR020A   ../wvel/${t}_${Y01}${A}.a
      setenv CDF_TITLE $N
      setenv CDF_INST  "COAPS/FSU"
      setenv CDF_FILE  ${t}_${Y01}${A}.nc
      /bin/rm -rf $CDF_FILE
      ${BINRUN} ~/HYCOM-tools/force/src/force2nc
      if (-e ./SAVE) then
        foreach f ( ${t}_${Y01}${A}.nc )
          ln ${f} ./SAVE/${f}
        end
      endif
#   CICE_TYPE
