...
# --- awk -f 284.awk y01=098 ab=a 284P.csh > 284p098a.csh
C
...
setenv CDF071 ${N}-sea_${YC1}_01hr_precip.nc
setenv CDF072 ${N}-sea_${YCX}_01hr_precip.nc
C
touch  $CDF071
if (-z $CDF071) then
  ${pget} ${W}/$CDF071 . &
...
cat <<E-o-D  > fort.05i
 ...
 &AFTIME
  ...
  TSTART = ${TS},
  TMAX   = ${TM},
  PARMIN =     0.0,   !precip is +ve into ocean
  PARMAX =  9999.0,   !disable parmax
  PAROFF =     0.0,   !no offset
  PARSCL =     0.001, !m/s    into ocean
 /
 &AFFLAG
...
E-o-D
C
C --- run the precip interpolation.
C
date +"precip %c"
...
C --- Output.
C
/bin/mv fort.10  precip_${Y01}${A}.b
/bin/mv fort.10a precip_${Y01}${A}.a
C
if (-e ./SAVE) then
...
#   global tripole region
  ...
  cd ../cice
  if (CICE_TYPE == 0) then
    /bin/rm precip_${Y01}${A}.r
    ${BINRUN} ~/HYCOM-tools/bin/hycom2raw8 ../pcip/precip_${Y01}${A}.a ${IDM} ${JDM} 1 1 ${IDM} ${JDA} precip_${Y01}${A}.r >! precip_${Y01}${A}.B
    if (-e ./SAVE) then
      foreach f ( precip_${Y01}${A}.[rB] )
        ln ${f} ./SAVE/${f}
      end
    endif
  else
    touch  regional.grid.a regional.grid.b
    if (-z regional.grid.b) then
      ln -f ../pcip/regional.grid.b regional.grid.b
      ln -f ../pcip/regional.grid.a regional.grid.a
    endif
    setenv FOR020    ../pcip/precip_${Y01}${A}.b
    setenv FOR020A   ../pcip/precip_${Y01}${A}.a
    setenv CDF_TITLE $N
    setenv CDF_INST  "COAPS/FSU"
    setenv CDF_FILE  precip_${Y01}${A}.nc
    /bin/rm -rf $CDF_FILE
    ${BINRUN} ~/HYCOM-tools/force/src/force2nc
    if (-e ./SAVE) then
      foreach f ( precip_${Y01}${A}.nc )
        ln ${f} ./SAVE/${f}
      end
    endif
  endif
...
C
C  --- END OF JUST IN TIME PRECIP GENERATION SCRIPT.
C
date +"END    %c"
