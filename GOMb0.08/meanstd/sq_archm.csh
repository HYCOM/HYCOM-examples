...
C
#
# --- form the mean-square of a sequence of archive files
# --- R is region name.
# --- E is expt number.
...
#
limit
if ( ! -e hycom_mnsq ) /bin/cp ${SRC}/meanstd/src/hycom_mean ${STMP}
#
...
#
  echo $YRFLAG $ARCHFQ $DF $DL | ${SRCBIN}/bin/hycom_archm_dates | tail -n +3 >! ${E}_mean.tmp
...
#
  ...
  if (${ARCHIVE} == 1) then
    awk -f ${M}/mn+sq.awk y=${Y} p=${P} ex=${EX} r=${R} x=${X} \
     ${M}/sq_rcp_A.csh >! sq_rcp_A_${Y}${P}.csh
     ${QSUBMIT} sq_rcp_A_${Y}${P}.csh
  endif
end
#
